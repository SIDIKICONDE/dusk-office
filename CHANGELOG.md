...
- **Fixed**: **Removed 19 invalid color keys** flagged by the VS Code color-theme schema (`dialog.*`, `dialogShadow`, `editorWidget.shadow`, `inlineEdit.background`/`border`/`foreground`/`insertedTextBackground`/`removedTextBackground`, `notificationErrorBackground`, `notificationWarningBackground`, `notificationInfoBackground`, `tab.groupBorder`) — these were never registered in VS Code's `ColorRegistry` and were silently ignored; dialogs reuse `editorWidget.*` colors, notifications use `notificationsErrorIcon.foreground` & co., and shadows use `widget.shadow`. Zero schema warnings remain on `theme-sources/dusk.json`.
...
- **Added**: **Web extension support (vscode.dev / github.dev)** — the full runtime now activates in the browser-based extension host, not just desktop. Bundled with **esbuild** into `dist/node/extension.js` (`main`) and `dist/web/extension.js` (`browser`) via `vscode:prepublish`. To make the runtime filesystem-free, theme data is now embedded at build time (`scripts/build-themes-bundle.mjs` → `lib/generated/themes-bundle.js`, regenerated inside `make:full`): the **Theme Gallery** and both contrast verifiers read the embedded bundle instead of reading `themes/*.json` from disk, and **workspace fingerprint** reads project manifests through `vscode.workspace.fs` (Uri-based) instead of Node `fs`. The `fs`/`path` include-chain flattening was split out into Node-only `lib/terminal/theme-merge.js` and `lib/themes/theme-merge-data.js` (used only by scripts, tests, and the bundle generator) so the runtime graph bundles cleanly for the Web Worker host. CI builds both targets (`npm run build:ext`); VSIX now ships `dist/` instead of the CommonJS source.
- **Added**: **Theme Gallery** — new `Dusk Office: Theme Gallery` command (and Control Center entry) opening a webview that renders every variant as a live card from its *real* palette (mini editor with comment/keyword/string/function/number/type tokens, title bar, activity bar, status bar, and terminal ANSI swatches). Clicking **Apply** switches theme instantly. New module `lib/themes/theme-data.js` flattens `colors` + `tokenColors` across the `include` chain and builds the preview model; webview uses a CSP + per-load nonce. Registered via `duskOffice.openThemeGallery`.
- **Added**: **Editor & UI contrast verification** — new `Dusk Office: Verify Editor & UI Contrast` command (and Control Center entry) plus `scripts/verify-ui-contrast.mjs` (`npm run verify:ui`). Scores editor text, syntax tokens, and workbench chrome (status bar, tabs, buttons, badges, lists, inputs, notifications, diagnostics) against WCAG AA — 4.5:1 for body text, 3:1 for UI components and a 3:1 readability floor for syntax tokens. Reuses the same luminance/contrast math as the terminal checker (`lib/contrast/ui-contrast.js`). Added to the `validate` gate so UI-contrast regressions fail the build.
- **Fixed**: **WCAG contrast across all 27 themes** — 19 contrast violations corrected by the new idempotent `scripts/fix-ui-contrast.mjs` pass (wired as the final step of `make:full`, before `validate`):
  - **Badges**: translucent `badge.background` / `activityBarBadge.background` chips baked opaque so their ink reads (e.g. base `#5a8fb0aa` → `#5a8fb0`, propagated to all dark variants; Corporate, Midnight, Mist, Nebula, Neon, Dawn, Ledger lifted to ≥ 3:1).
  - **Diagnostic glyphs**: low-contrast `editorWarning.foreground` / `editorError.foreground` on light themes darkened while preserving hue (e.g. Ivory warning `#c9a85c` → `#957c44`).
  - **Syntax**: sub-3:1 comments and tokens raised to the readability floor (Corporate comment, Ledger comment/string/type/number).
  - The fix edits each color in the file that *owns* it (respecting the `include` chain), is deterministic and idempotent, so regenerated `themes/` stays in sync with `theme-sources/` and `verify-themes-fresh` remains green.
- **Added**: **JetBrains export — JSX/HTML syntax tokens** — `component` and `jsxTag` keys added to `SYNTAX_SCOPE_TARGETS` in `lib/export/theme-export-palette.mjs` so the export pipeline extracts distinct colors for `support.class.component`, `entity.name.function.component`, `punctuation.definition.tag.jsx/tsx`, and `meta.tag.jsx/tsx`. Five new `.icls` attribute mappings in `lib/export/export-formats.mjs`: `HTML_TAG` (jsxTag color), `HTML_TAG_NAME`, `HTML_ATTRIBUTE_NAME`, `HTML_ATTRIBUTE_VALUE`, and `JSX_UNOFFICIAL_TAG_NAME` (component color, bold). Fallback colors added for dark and light palettes. Previously, React component names and JSX punctuation fell into generic `DEFAULT_TAG`/`DEFAULT_CLASS_NAME` — they now render with their own VS Code–derived colors.
- **Added**: tests — `tests/theme-data.test.js`, `tests/ui-contrast.test.js`, `tests/contrast-fix.test.js` (theme-data flattening, scope matching, UI/syntax scoring, hue-preserving fixes, idempotency; 217 unit tests total).
