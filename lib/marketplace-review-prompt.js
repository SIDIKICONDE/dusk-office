...
function getEngagementIncrement(trigger) {
  return ENGAGEMENT_WEIGHTS[trigger] ?? 0;
}
...
function shouldShowMarketplaceReviewPrompt({
  ...
  if (engagementCount < ENGAGEMENT_THRESHOLD) return false;
...
async function openMarketplaceReviewPage(options = {}) {
  ...
  const preferredItem = items.find((item) => item._entry.id === preferredId);
  const picked = await vscode.window.showQuickPick(items, {
    ...
    activeItems: preferredItem ? [preferredItem] : undefined,
  });
...
async function recordMarketplaceReviewEngagement(context, trigger) {
  try {
    if (!getMarketplaceReviewEnabled()) return;
    const increment = getEngagementIncrement(trigger);
    if (increment <= 0) return;

    const current = context.globalState.get(keys.MARKETPLACE_REVIEW_ENGAGEMENT_KEY) || 0;
    await context.globalState.update(keys.MARKETPLACE_REVIEW_ENGAGEMENT_KEY, current + increment);
    await maybeShowMarketplaceReviewPrompt(context);
  } catch (err) {
    log.error("recordMarketplaceReviewEngagement", err);
  }
}
...
async function maybeShowMarketplaceReviewPrompt(context) {
  try {
    ...
    const engagementCount = context.globalState.get(keys.MARKETPLACE_REVIEW_ENGAGEMENT_KEY) || 0;
...
module.exports = {
  ...
  recordMarketplaceReviewEngagement,
