...
function shouldColorizeDocument(doc) {
  ...
  if (cfg.get("editorAnsi.allLanguages", false)) return true;
...
/** Maximum number of distinct decoration types to prevent unbounded growth
 *  on files with many unique ANSI style combinations. Once reached, new
 *  styles fall back to the default (no decoration).
 */
const MAX_DECORATION_TYPES = 256;
...
class AnsiEditorSupport {
  ...
  async refresh(editor) {
    ...
    for (const [key, ranges] of buckets) {
      ...
      if (!type) {
        // Cap decoration types to prevent unbounded growth on pathological files.
        if (this.types.size >= MAX_DECORATION_TYPES) {
          // Reuse an existing type that won't be used this round (evicted below).
          // If all existing types are still in use, skip this style.
          continue;
        }
