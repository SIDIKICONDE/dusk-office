...
async function detectWorkspaceFingerprint(context, options = {}) {
  try {
    ...
    let signals;
    try {
      signals = await collectWorkspaceSignals(folders[0].uri);
      for (let i = 1; i < folders.length; i++) {
        mergeFingerprintSignals(signals, await collectWorkspaceSignals(folders[i].uri));
      }
    } catch {
      return null;
    }
    ...
    if (ranked.length === 0) {
      if (options.force) {
        void vscode.window.showInformationMessage(
          'Dusk Office: no strong workspace match detected. Use "Dusk Office: Choose Theme" to pick one manually.',
        );
      }
      return null;
    }
