...
/**
 * File reads go through `vscode.workspace.fs` (Uri-based) rather than Node's
 * `fs`, so workspace fingerprinting works in the **web** extension host where
 * Node's filesystem is unavailable. Same 256 KB cap to stay fast on big configs.
 */
...
/** Shallow async JSON read with the same cap. */
 ...
 * Now async to avoid blocking the extension host thread on I/O.
 */
async function collectWorkspaceSignals(rootUri) {
  const signals = {
    npmDeps: new Set(),
    npmKeywords: [],
    npmText: "",
  ...
  if (!rootUri) return signals;
  const child = (name) => vscode.Uri.joinPath(rootUri, name);

  // package.json
  const pkg = await readJsonFile(child("package.json"));
  ...
  // Cargo.toml
  const cargo = await readTextFile(child("Cargo.toml"));
  if (cargo) {
  ...
  // Python: pyproject.toml + requirements.txt — read in parallel
  const [pyprojectText, requirementsText, pipfileText] = await Promise.all([
    readTextFile(child("pyproject.toml")),
    readTextFile(child("requirements.txt")),
    readTextFile(child("Pipfile")),
  ]);
  signals.pythonText =
    [pyprojectText || "", requirementsText || "", pipfileText || ""]
      .join("\n")
      .toLowerCase();

  // Go
  signals.goText = ((await readTextFile(child("go.mod"))) || "").toLowerCase();

  // PHP / Composer
  const composer = await readJsonFile(child("composer.json"));
  ...
  try {
    ...
    // Read YAML files in parallel for K8s manifest detection
    const yamlChecks = [];
    for (const [name, fileType] of entries) {
      const isFile = (fileType & vscode.FileType.File) !== 0;
      const isDirectory = (fileType & vscode.FileType.Directory) !== 0;
      if (isFile) {
        signals.files.add(name.toLowerCase());
        // path.extname semantics: no extension for dotfiles like ".storybook".
        const dot = name.lastIndexOf(".");
        const ext = dot > 0 ? name.slice(dot).toLowerCase() : "";
        if (ext) signals.extensionCounts[ext] = (signals.extensionCounts[ext] || 0) + 1;

        if (ext === ".yaml" || ext === ".yml") {
          yamlChecks.push(readTextFile(child(name)).then((text) => ({ text, name })));
        }
      } else if (isDirectory) {
        signals.files.add(name.toLowerCase() + "/");
      }
    }
    const yamlResults = await Promise.all(yamlChecks);
    for (const { text } of yamlResults) {
      if (
        text &&
