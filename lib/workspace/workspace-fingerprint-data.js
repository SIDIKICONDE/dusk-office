...
/** Shallow async JSON read with a 256 KB cap to stay fast and avoid huge configs. */
...
/** Plain-text async read with the same 256 KB cap. */
 ...
 */
async function collectWorkspaceSignals(rootDir) {
  const signals = {
    npmDeps: new Set(),
    npmKeywords: [],
    npmText: "",
  ...
  // Cargo.toml
  const cargo = await readTextFile(path.join(rootDir, "Cargo.toml"));
  if (cargo) {
  ...
  // Go
  signals.goText = ((await readTextFile(path.join(rootDir, "go.mod"))) || "").toLowerCase();

  // PHP / Composer
  const composer = await readJsonFile(path.join(rootDir, "composer.json"));
  ...
  try {
    ...
    for (const e of entries) {
      if (e.isFile()) {
        ...
        if (ext === ".yaml" || ext === ".yml") {
          yamlChecks.push(
            readTextFile(path.join(rootDir, e.name)).then((text) => ({
              text,
              name: e.name,
            }))
          );
