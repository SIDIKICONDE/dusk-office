...
function markGalleryOpen(open) {
  state.isThemeGalleryOpen = open;
}
...
<body>
  ...
  <p class="lead">${cards.length} variants from their real palettes. Cards are a visual preview only — the workbench (sidebar, activity bar, editor) stays put until you click <strong>Apply</strong>.</p>
...
function makeNonce() {
  ...
  let out = "";
  for (let i = 0; i < 24; i += 1) out += chars[bytes[i] % chars.length];
  return out;
}
