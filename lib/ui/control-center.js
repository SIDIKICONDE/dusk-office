...
const { THEME_VARIANTS, getThemeDisplayLabel, getThemeKindLabel, isDuskTheme, isThemeName } = require("../themes/theme-common.js");
...
const { normalizeTimezone, isValidTimezone } = require("../themes/schedule-time.js");
...
function separator(label) {
  return { label, kind: vscode.QuickPickItemKind.Separator, action: null };
}

function buildControlCenterItems(context) {
    ...
    },
    {
      ...
      action: async () => {
        try {
          ...
          const timezoneRaw = await vscode.window.showInputBox({
            title: "Auto Switch: Timezone (IANA)",
            prompt: "e.g. Europe/Paris. Empty or local = machine time.",
            value: autoSwitch.timezone || "",
            validateInput: (text) => {
              const trimmed = String(text ?? "").trim();
              if (!trimmed || trimmed.toLowerCase() === "local") return undefined;
              return isValidTimezone(trimmed)
                ? undefined
                : "Enter a valid IANA timezone (e.g. Europe/Paris) or leave empty";
            },
          });
          if (timezoneRaw === undefined) return;
          ...
          await Promise.all([
            ...
            cfg.updateConfigValue(extCfg, "autoSwitch.timezone", timezone),
          ]);
          void vscode.window.showInformationMessage(
            `Auto switch configured: ${lightTheme} @${lightHour}h / ${darkTheme} @${darkHour}h · ${formatScheduleTimezone(timezone)}`,
          );
...
async function openControlCenter(context) {
  try {
    const picked = await vscode.window.showQuickPick(buildControlCenterItems(context), {
      title: "Dusk Office",
...
module.exports = { openControlCenter, buildControlCenterItems };
