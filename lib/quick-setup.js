...
const { recordMarketplaceReviewEngagement } = require("./marketplace-review-prompt.js");
 ...
 *
 *  1. Pick a favorite variant (live preview)
  ...
  try {
    ...
    if (mode._mode === adaptive) {
      ...
      void vscode.window.showInformationMessage(
        `Quick Setup complete — Adaptive Focus is on. Current: ${cfg.getCurrentTheme()}.`,
      );
      ...
      void vscode.window.showInformationMessage(
        `Quick Setup complete — Auto Day/Night is on. Current: ${cfg.getCurrentTheme()}.`,
      );
    } else {
      void vscode.window.showInformationMessage(
        `Quick Setup complete — manual mode. Current: ${current}.`,
      );
    }

    // Step 3 — persist as favorite
    await themes.persistFavoriteTheme(context, cfg.getCurrentTheme());
    void recordMarketplaceReviewEngagement(context, "quickSetup");
  } catch (err) {
