    ...
    .then(async (selection) => {
      ...
      const tryClear = async (config, key, target) => {
        ...
        try {
          ...
          failures.push({ key, target, reason });
          return false;
        }
      };
      ...
      try {
        ...
        const stateClears = [
          ...
          [context.globalState, keys.WALKTHROUGH_SHOWN_KEY],
          [context.globalState, keys.MARKETPLACE_REVIEW_COMPLETED_KEY],
          [context.globalState, keys.MARKETPLACE_REVIEW_DISMISSED_KEY],
          [context.globalState, keys.MARKETPLACE_REVIEW_SESSION_COUNT_KEY],
          [context.globalState, keys.MARKETPLACE_REVIEW_FIRST_ACTIVATION_KEY],
          [context.globalState, keys.MARKETPLACE_REVIEW_COMPLETED_VERSION_KEY],
          [context.globalState, keys.PROFILE_DEFAULTS_MIGRATION_KEY],
        ...
        vscode.window.showErrorMessage(`Dusk Office reset failed: ${failMessage}`, "OK");
      }
    });
}
