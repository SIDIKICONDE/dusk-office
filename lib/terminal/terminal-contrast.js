 ...
 * @param {{ minFgRatio?: number; minAnsiRatio?: number }} [options]
 */
function checkTerminalContrast(colors, uiTheme, options = {}) {
  const minFgRatio = options.minFgRatio ?? MIN_TERMINAL_FG_RATIO;
  const minAnsiRatio = options.minAnsiRatio ?? MIN_TERMINAL_ANSI_RATIO;
  const failures = [];
  ...
  if (typeof fgValue === "string") {
    ...
    if (fgRgb) {
      ...
      if (ratio < minFgRatio) {
        failures.push(
          `terminal.foreground ${fgValue} vs terminal.background ${bgValue}: ${ratio.toFixed(2)}:1 (min ${minFgRatio}:1)`,
        );
      }
    }
  }
  ...
  for (const key of TERMINAL_ANSI_KEYS) {
    ...
    if (!fgRgb) continue;
    const ratio = contrastRatio(luminance(fgRgb), bgLum);
    if (ratio < minAnsiRatio) {
      failures.push(`${key} ${colorValue}: ${ratio.toFixed(2)}:1 (min ${minAnsiRatio}:1)`);
    }
  }
