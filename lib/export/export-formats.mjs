...
/**
 * Serialize a palette as a resolved VS Code color-theme JSON document.
 *
 ...
 * @param {import('./theme-export-palette.mjs').ExportPalette} p
 */
export function toGhosttyConf(p) {
  const { editor, terminal } = p;
  ...
  return `# ${p.name} — generated from Dusk Office (VS Code theme pack)
# Install: place in ~/.config/ghostty/themes/ or ~/.config/ghostty/config and set
#   theme = ${p.slug}
background = ${terminal.background}
foreground = ${terminal.foreground}
cursor-color = ${editor.cursor ?? terminal.foreground}
cursor-text = ${terminal.background}
selection-background = ${editor.selection ?? terminal.foreground}
selection-foreground = ${terminal.background}
palette = ${palette.map((c, i) => `${i}=${c}`).join(",")}
`;
}
 ...
 * @param {import('./theme-export-palette.mjs').ExportPalette} p
 */
export function toWezTermToml(p) {
  const { editor, terminal } = p;
  const a = terminal.ansi;
  const q = (v) => `"${v}"`;
  return `# ${p.name} — generated from Dusk Office (VS Code theme pack)
# Install: copy to ~/.config/wezterm/colors/${p.slug}.toml then in wezterm.lua:
#   color_scheme = "${p.slug}"
[colors]
foreground = ${q(terminal.foreground)}
background = ${q(terminal.background)}
cursor_bg = ${q(editor.cursor ?? terminal.foreground)}
cursor_fg = ${q(terminal.background)}
cursor_border = ${q(editor.cursor ?? terminal.foreground)}
selection_bg = ${q(editor.selection ?? terminal.foreground)}
selection_fg = ${q(terminal.background)}
ansi = [
  ${q(a.black)}, ${q(a.red)}, ${q(a.green)}, ${q(a.yellow)},
  ${q(a.blue)}, ${q(a.magenta)}, ${q(a.cyan)}, ${q(a.white)},
]
brights = [
  ${q(a.brightBlack)}, ${q(a.brightRed)}, ${q(a.brightGreen)}, ${q(a.brightYellow)},
  ${q(a.brightBlue)}, ${q(a.brightMagenta)}, ${q(a.brightCyan)}, ${q(a.brightWhite)},
]
`;
}
 ...
 * @param {import('./theme-export-palette.mjs').ExportPalette} p
 */
export function toWarpYaml(p) {
  const { editor, terminal } = p;
  const a = terminal.ansi;
  return `# ${p.name} — generated from Dusk Office (VS Code theme pack)
# Install: copy to ~/.warp/themes/${p.slug}.yaml then pick it in Warp settings
accent: '${editor.cursor ?? terminal.foreground}'
background: '${terminal.background}'
foreground: '${terminal.foreground}'
details: darker
cursor: '${editor.cursor ?? terminal.foreground}'
selection: '${editor.selection ?? terminal.foreground}'
terminal_colors:
  normal:
    black: '${a.black}'
    red: '${a.red}'
 ...
 * @param {import('./theme-export-palette.mjs').ExportPalette} p
 */
export function toWindowsTerminalJson(p) {
  const { editor, terminal } = p;
  const a = terminal.ansi;
  return JSON.stringify(
    {
      ...
      background: terminal.background,
      foreground: terminal.foreground,
      cursorColor: editor.cursor ?? terminal.foreground,
      selectionBackground: editor.selection ?? terminal.foreground,
      black: a.black,
      red: a.red,
      green: a.green,
      yellow: a.yellow,
...
export function toKittyConf(p) {
  ...
  return `# ${p.name} — generated from Dusk Office (VS Code theme pack)
# Install: place in ~/.config/kitty/themes/${p.slug}.conf and add
#   include themes/${p.slug}.conf
# to your kitty.conf
background ${terminal.background}
foreground ${terminal.foreground}
cursor ${editor.cursor ?? terminal.foreground}
cursor_text_color ${terminal.background}
selection_background ${editor.selection ?? terminal.foreground}
selection_foreground ${terminal.background}
 ...
 * @param {import('./theme-export-palette.mjs').ExportPalette} p
 */
export function toKonsoleColorScheme(p) {
  const { terminal } = p;
  const a = terminal.ansi;
  // #rrggbb → "r,g,b" (format Konsole : 0-255 séparés par virgules)
  const rgb = (hex) => {
  ...
  const block = (name, c) => `[${name}]\nColor=${rgb(c)}`;
  const lines = normals.map((c, i) => `${block(`Color${i}`, c)}\n\n${block(`Color${i}Intense`, brights[i])}`).join("\n\n");
  return `[General]
Description=${p.name}
Blur=true
Opacity=1
...
[Background]
Color=${rgb(terminal.background)}

[BackgroundIntense]
Color=${rgb(terminal.background)}

[Foreground]
Color=${rgb(terminal.foreground)}
...
export function toJetBrainsIcls(p) {
  const { editor, syntax, terminal, workbenchUi, codeLens } = p;
  const u = workbenchUi;
  const ansi = terminal.ansi;
  const jetFg = (hex) => stripHash(hex);
  const codeLensFg = codeLens ?? syntax.comment ?? editor.foreground;
  const metadataFg = p.type === "light" ? codeLensFg : syntax.attribute;
  const sidebar = uiGet(u, "sideBar.background", p.ui.sidebar ?? editor.background);
  const panel = uiGet(u, "panel.background", p.ui.panel ?? editor.background);
  const tabActive = uiGet(u, "tab.activeBackground", p.ui.tabActive ?? editor.background);
...
<scheme name="${escapeXml(p.name)}" version="142" parent_scheme="${p.type === "light" ? "Default" : "Darcula"}">
  ...
  <colors>
    ...
    <option name="SELECTION_FOREGROUND" value="${jetFg(editor.foreground)}"/>
    <option name="CARET_ROW_COLOR" value="${jetFg(editor.lineHighlight ?? editor.background)}"/>
  ...
  <attributes>
    ...
    <option name="DEFAULT_METADATA">
      <value><option name="FOREGROUND" value="${jetFg(metadataFg)}"/></value>
    </option>
    ...
    <option name="HTML_TAG">
      <value><option name="FOREGROUND" value="${jetFg(syntax.jsxTag ?? syntax.tag)}"/></value>
    </option>
    ...
    <option name="JSX_UNOFFICIAL_TAG_NAME">
      <value><option name="FOREGROUND" value="${jetFg(syntax.component ?? syntax.type)}"/><option name="FONT_TYPE" value="1"/></value>
    </option>
...
${reworked.attributes}
    ...
    <option name="CODE_LENS_BORDER_COLOR">
      <value><option name="EFFECT_COLOR" value="${jetFg(codeLensFg)}"/></value>
    </option>
    <option name="CODE_VISION_INLAY">
      <value><option name="FOREGROUND" value="${jetFg(codeLensFg)}"/></value>
    </option>
    <option name="CODE_VISION_INLAY_HOVERED">
      <value><option name="FOREGROUND" value="${jetFg(editor.foreground)}"/><option name="EFFECT_TYPE" value="1"/></value>
    </option>
