...
/** First-install Apply — Finance is the listed flagship variant (base stays out of pickers). */
const DEFAULT_ONBOARDING_THEME = THEME_VARIANTS[0];
...
function isLightThemeVariant(theme) {
  return typeof theme === "string" && LIGHT_THEME_VARIANTS.has(stripThemeDisplayLabel(theme));
}
...
function getThemeDisplayLabel(theme) {
  const plain = typeof theme === "string" ? stripThemeDisplayLabel(theme) : theme;
  if (isThemeBase(plain)) return `${DARK_THEME_INSIGNIA} ${plain} · Base`;
  const insignia = getThemeInsignia(plain);
  return insignia ? `${insignia} ${plain}` : plain;
}
...
function isThemeBase(theme) {
  return typeof theme === "string" && stripThemeDisplayLabel(theme) === THEME_BASE;
}

function isDuskTheme(theme) {
  return typeof theme === "string" && ALL_DUSK_THEMES.includes(stripThemeDisplayLabel(theme));
}
...
function resolveAdaptiveLanguageRules(languageOverrides) {
  ...
  for (const [rawLang, rule] of Object.entries(languageOverrides)) {
    ...
    const light = coerceOptionalDuskTheme(rule.light);
    const dark = coerceOptionalDuskTheme(rule.dark);
    if (light) base.light = light;
    if (dark) base.dark = dark;
...
function getThemeShortLabel(theme) {
  ...
  const plain = stripThemeDisplayLabel(theme);
  if (isThemeBase(plain)) return `${DARK_THEME_INSIGNIA} Base`;
  const base = plain.replace(/^Dusk Office\s*/, "") || "Dusk";
  const insignia = getThemeInsignia(plain);
 ...
 * Used by `lib/core/configuration.js` so the rest of the extension can trust the   *
...
function coerceDuskTheme(value, fallback) {
  ...
  const plain = stripThemeDisplayLabel(value);
  return ALL_DUSK_THEMES.includes(plain) ? plain : fallback;
}

function coerceOptionalDuskTheme(value) {
  if (typeof value !== "string") return "";
  const plain = stripThemeDisplayLabel(value);
  return ALL_DUSK_THEMES.includes(plain) ? plain : "";
}
...
function sanitizeAdaptiveFocusConfig(raw, scheduleTimezone = "") {
  const src = raw && typeof raw === "object" ? raw : {};
  const timezone = normalizeTimezone(src.timezone) || normalizeTimezone(scheduleTimezone);
  return {
    enabled: coerceBoolean(src.enabled, false),
...
function computeAutoSwitchTheme(autoSwitch, now = new Date()) {
  ...
  const hour = resolveScheduleHour(now, autoSwitch.timezone);
  const useDark =
...
function computeAdaptiveFocusTheme(languageId, now = new Date(), options = {}, cfg) {
  ...
  const hour = resolveScheduleHour(now, cfg.timezone);
  if (
 ...
 * @param {string} [opts.defaultLightTheme]
 * @param {string} [opts.defaultDarkTheme]
 * @param {Record<string, { light?: string; dark?: string }>} [opts.languageOverrides]
 */
