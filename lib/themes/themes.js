...
/**
 * Live preview only — updates the active color theme without workspace memory,
 * previous-theme tracking, title-bar sync, or manual-override grace.
 */
async function previewTheme(theme) {
  try {
    ...
    await cfg.updateConfigValue(workbenchCfg, "colorTheme", getThemeDisplayLabel(plainName));
    await syncPreferredThemeIfAutoDetect(plainName, workbenchCfg);
    return true;
  } catch (err) {
    log.warn("previewTheme", err);
    return false;
  }
}
...
async function setThemeVariant(context) {
  ...
  return new Promise((resolve) => {
    ...
    quickPick.onDidChangeActive((active) => {
      try {
        const item = /** @type {{ _theme?: string }} */ (active[0]);
        if (!item || !item._theme || item._theme === lastPreviewed) return;
        lastPreviewed = item._theme;
        void cfg.updateConfigValue(workbenchCfg, "colorTheme", getThemeDisplayLabel(item._theme)).catch((err) =>
    ...
    quickPick.onDidAccept(async () => {
      try {
        accepted = true;
        const item = /** @type {{ _theme?: string }} */ (quickPick.activeItems[0]);
        quickPick.hide();
        if (!item || !item._theme) {
          resolve();
          return;
