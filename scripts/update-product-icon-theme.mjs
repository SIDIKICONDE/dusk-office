...
// Read existing theme to get current icon definitions
const themePath = path.join(root, 'producticons/dusk-office-product-icon-theme.json');
const theme = JSON.parse(fs.readFileSync(themePath, 'utf8'));

// Read generated SVGs to get icon names
const srcDir = path.join(root, 'icons-src');
const svgFiles = fs.readdirSync(srcDir).filter(f => f.endsWith('.svg'));
const iconNames = svgFiles.map(f => f.replace('.svg', ''));

// Codepoints mapping (same as .fantasticonrc.js)
const codepoints = {
  files: 0xEBDF,
  search: 0xEC0D,
...
// Build new iconDefinitions
const iconDefinitions = {};
for (const name of iconNames) {
  const codepoint = codepoints[name];
  if (codepoint) {
    iconDefinitions[name] = {
      fontCharacter: '\\' + codepoint.toString(16).toLowerCase(),
...
// Update theme
theme.iconDefinitions = iconDefinitions;

// Write back
fs.writeFileSync(themePath, JSON.stringify(theme, null, 2));
console.log('Updated product icon theme with', Object.keys(iconDefinitions).length, 'icons');
