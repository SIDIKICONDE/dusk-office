...
/** Ancienne paire (texte cyan sur fond cyan très transparent) — bouton « Install » illisible. */
const LEGACY_EXTENSION_BUTTON_FG = "#22d3ee";
const LEGACY_EXTENSION_BUTTON_BG = "#06b6d433";
...
/**
 * Couleurs effectives après chaîne `include` (parent puis enfant, comme VS Code).
 * @param {string} file
 */
function getEffectiveColors(file) {
  const theme = readThemeJson(file);
  let base = {};
  if (theme.include) {
    const inc = resolveInclude(file, theme.include);
    base = getEffectiveColors(inc);
  }
 ...
 * @param {string} label
 */
function assertExtensionButtonLegible(themeFile, label) {
  const colors = getEffectiveColors(themeFile);
  const fg = colors["extensionButton.prominentForeground"];
  const bg = colors["extensionButton.prominentBackground"];
  if (bg === LEGACY_EXTENSION_BUTTON_BG) {
    throw new Error(
      `${label} (${path.relative(root, themeFile)}): extensionButton.prominentBackground ne doit pas rester ${LEGACY_EXTENSION_BUTTON_BG} (contraste insuffisant).`,
    );
  }
  if (fg === LEGACY_EXTENSION_BUTTON_FG) {
    throw new Error(
      `${label} (${path.relative(root, themeFile)}): extensionButton.prominentForeground ne doit pas rester ${LEGACY_EXTENSION_BUTTON_FG} (texte trop clair sur fond accent).`,
    );
  }
}
...
function main() {
  ...
  for (const t of themes) {
    ...
    assertExtensionButtonLegible(full, t.label ?? t.path);
  }
