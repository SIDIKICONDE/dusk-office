...
Picks a variant from the **active editor language** and the **current hour**. Logic lives in **`lib/themes/theme-common.js`** (`ADAPTIVE_LANGUAGE_RULES`). Configure day hours, defaults, and per-language overrides in Settings (`duskOffice.adaptiveFocus.*`).
